...
def measure_smoothness_costs_old(mwe, next_frame_streaks, alignments, next_frame,
    ...
    """Measure smoothness costs
    
    Returns: smoothness_costs, smoothness_costs_by_alignment
    """
    # Data for each streak to fit, in next_frame only
    streak_data = mwe.loc[
        mwe['streak'].isin(next_frame_streaks) &
    ...
    if method == 'fast':
        ...
        # DataFrame it
        smoothness_costs_by_alignment = pandas.DataFrame.from_dict({
            'dist': vals.values,
        ...
        # DataFrame
        smoothness_costs_by_alignment = pandas.DataFrame(
    ...
    # Mean over each alignment
    smoothness_costs = smoothness_costs_by_alignment.sum(axis=1)
    
    # Normalize
    smoothness_costs = smoothness_costs / 15
    smoothness_costs.name = 'smoothness'    
    
    return smoothness_costs, smoothness_costs_by_alignment
